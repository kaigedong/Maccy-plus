...
class HistoryItemTests: XCTestCase {
    ...
    func testImageFromUniversalClipboard() throws {
        let url = try XCTUnwrap(Bundle(for: type(of: self)).url(forResource: "guy", withExtension: "jpeg"))
        let fileURLContent = HistoryItemContent(
            type: NSPasteboard.PasteboardType.fileURL.rawValue,
            value: url.dataRepresentation
        )
        ...
        XCTAssertEqual(item.image?.tiffRepresentation, try NSImage(data: Data(contentsOf: url))?.tiffRepresentation)
    }
