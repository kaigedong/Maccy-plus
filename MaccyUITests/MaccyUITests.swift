...
class MaccyUITests: XCTestCase {
    ...
    func testSearchFiles() throws {
        ...
        try assertExists(items[XCTUnwrap(file2.absoluteString.removingPercentEncoding)])
        try assertNotExists(items[XCTUnwrap(file1.absoluteString.removingPercentEncoding)])
    }
    ...
    func testCopyImage() throws {
        ...
        try assertPasteboardDataCountEquals(XCTUnwrap(image2.tiffRepresentation?.count), forType: .tiff)
    }
    ...
    func testCopyFile() throws {
        ...
        XCTAssertEqual(itemTitles[0 ... 1], try [
            XCTUnwrap(file1.absoluteString.removingPercentEncoding),
            XCTUnwrap(file2.absoluteString.removingPercentEncoding),
        ])

        try items[XCTUnwrap(file2.absoluteString.removingPercentEncoding)].firstMatch.click()
    ...
    func testOpenAndClose() throws {
        ...
        let cDown = try XCTUnwrap(CGEvent(keyboardEventSource: nil, virtualKey: CGKeyCode(kVK_ANSI_C), keyDown: true))
        ...
        let cUp = try XCTUnwrap(CGEvent(keyboardEventSource: nil, virtualKey: CGKeyCode(kVK_ANSI_C), keyDown: false))
        ...
        let shiftUp = try XCTUnwrap(CGEvent(keyboardEventSource: nil, virtualKey: CGKeyCode(kVK_Shift), keyDown: false))
        ...
        let commandUp = try XCTUnwrap(CGEvent(keyboardEventSource: nil, virtualKey: CGKeyCode(kVK_Command), keyDown: false))
        commandUp.flags = []
    ...
    func testOpenAndSelectSecondItem() throws {
        ...
        let modifiersUp = try XCTUnwrap(CGEvent(keyboardEventSource: nil, virtualKey: CGKeyCode(kVK_Shift), keyDown: false))
        modifiersUp.flags = []
    ...
    private func search(_ string: String) {
        ...
        for item in string {
            app.typeKey("\(item)", modifierFlags: [])
        }
        waitForSearch()
    }
