...
class HistoryItem {
    ...
    func generateTitle() -> String {
        ...
        var title = previewableText.shortened(to: 1000)
    ...
    var modified: Int? {
        guard let data = contentData([.modified]),
              let modified = String(data: data, encoding: .utf8)
        else {
            return nil
        }
    ...
    var fromMaccy: Bool {
        contentData([.fromMaccy]) != nil
    }

    var universalClipboard: Bool {
        contentData([.universalClipboard]) != nil
    }

    private var universalClipboardImage: Bool {
        universalClipboard && fileURLs.first?.pathExtension == "jpeg"
    }
    ...
    private func recognizeTextHandler(request: VNRequest, error _: Error?) {
        ...
        title = recognizedStrings.joined(separator: "\n")
    }
}
