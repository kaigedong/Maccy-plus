...
enum KeyChord: CaseIterable {
    ...
    static var deleteKey: Key? {
        Sauce.shared.key(shortcut: .delete)
    }
    ...
    static var previewKey: Key? {
        Sauce.shared.key(shortcut: .togglePreview)
    }

    static var previewModifiers: NSEvent.ModifierFlags? {
        KeyboardShortcuts.Shortcut(name: .togglePreview)?.modifiers
    }
